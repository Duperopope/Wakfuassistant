...
/// Chemin de la base craft_economy (HDV agrégé + noms items).
/// Cherche d'abord en mode dev (relatif au CWD = racine projet),
/// sinon dans app_data_dir (production).
pub fn hdv_craft_db_path() -> PathBuf {
    let dev_path = PathBuf::from("logs/craft_economy.db");
    if dev_path.exists() {
        return dev_path;
    }
    app_data_dir().join("craft_economy.db")
}

/// Chemin de la base hdv_market (observations brutes).
pub fn hdv_market_db_path() -> PathBuf {
    let dev_path = PathBuf::from("logs/hdv_market.db");
    if dev_path.exists() {
        return dev_path;
    }
    app_data_dir().join("hdv_market.db")
}
