/*
 * Decompiled with CFR 0.152.
 */
import java.nio.ByteBuffer;

/*
 * Renamed from yE
 */
final class ye_1 {
    private short adx = 0;
    private final uy_1 amc = new uy_1();
    final /* synthetic */ yd_1 amd;

    ye_1(yd_1 yd_12) {
        this.amd = yd_12;
    }

    public void avW() {
        this.amd.adx = this.adx;
        this.amd.amb.ZC = this.amc.ZC;
        this.amd.amb.aaQ = this.amc.aaQ;
        this.amd.amb.aaa = this.amc.aaa;
        this.amd.amb.aeF = this.amc.aeF;
        this.amd.amb.aeG = this.amc.aeG;
        this.amd.amb.aeH = this.amc.aeH;
        this.amd.amb.aeI = this.amc.aeI;
        this.amd.amb.aeJ = this.amc.aeJ;
        this.amd.amb.aeK = this.amc.aeK;
        this.amd.amb.aeL = this.amc.aeL;
        this.amd.amb.aeM = this.amc.aeM;
    }

    private boolean cF(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 0);
        return bl;
    }

    private boolean cx(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 1);
        return bl;
    }

    private boolean cG(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 313);
        return bl;
    }

    private boolean cH(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 314);
        return bl;
    }

    private boolean cI(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 315);
        return bl;
    }

    private boolean cJ(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10003);
        return bl;
    }

    private boolean cL(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10023);
        return bl;
    }

    private boolean cM(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10028000);
        return bl;
    }

    private boolean cN(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10029000);
        return bl;
    }

    private boolean cO(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10032003);
        return bl;
    }

    private boolean cP(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10035004);
        return bl;
    }

    private boolean cQ(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10035007);
        return bl;
    }

    private boolean cR(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10036004);
        return bl;
    }

    private boolean cS(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10037001);
        return bl;
    }

    private boolean cT(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10040005);
        return bl;
    }

    private boolean cU(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10045010);
        return bl;
    }

    private boolean cC(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10048000);
        return bl;
    }

    private boolean cV(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10055001);
        return bl;
    }

    private boolean cW(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10064001);
        return bl;
    }

    private boolean cX(ByteBuffer byteBuffer) {
        this.adx = byteBuffer.getShort();
        boolean bl = this.amc.b(byteBuffer, 10066001);
        return bl;
    }

    public boolean b(ByteBuffer byteBuffer, int n) {
        if (n < 0) {
            return false;
        }
        if (n < 1) {
            boolean bl = this.cF(byteBuffer);
            return bl;
        }
        if (n < 313) {
            boolean bl = this.cx(byteBuffer);
            return bl;
        }
        if (n < 314) {
            boolean bl = this.cG(byteBuffer);
            return bl;
        }
        if (n < 315) {
            boolean bl = this.cH(byteBuffer);
            return bl;
        }
        if (n < 10003) {
            boolean bl = this.cI(byteBuffer);
            return bl;
        }
        if (n < 10023) {
            boolean bl = this.cJ(byteBuffer);
            return bl;
        }
        if (n < 10028000) {
            boolean bl = this.cL(byteBuffer);
            return bl;
        }
        if (n < 10029000) {
            boolean bl = this.cM(byteBuffer);
            return bl;
        }
        if (n < 10032003) {
            boolean bl = this.cN(byteBuffer);
            return bl;
        }
        if (n < 10035004) {
            boolean bl = this.cO(byteBuffer);
            return bl;
        }
        if (n < 10035007) {
            boolean bl = this.cP(byteBuffer);
            return bl;
        }
        if (n < 10036004) {
            boolean bl = this.cQ(byteBuffer);
            return bl;
        }
        if (n < 10037001) {
            boolean bl = this.cR(byteBuffer);
            return bl;
        }
        if (n < 10040005) {
            boolean bl = this.cS(byteBuffer);
            return bl;
        }
        if (n < 10045010) {
            boolean bl = this.cT(byteBuffer);
            return bl;
        }
        if (n < 10048000) {
            boolean bl = this.cU(byteBuffer);
            return bl;
        }
        if (n < 10055001) {
            boolean bl = this.cC(byteBuffer);
            return bl;
        }
        if (n < 10064001) {
            boolean bl = this.cV(byteBuffer);
            return bl;
        }
        if (n < 10066001) {
            boolean bl = this.cW(byteBuffer);
            return bl;
        }
        if (n < 10069001) {
            boolean bl = this.cX(byteBuffer);
            return bl;
        }
        return false;
    }
}

