...
extends fiu_0 {
    ...
    public byte[] aVX() {
        ...
        azg_1 azg_12 = new azg_1(this.eR(byArray));
        this.i(azg_12);
        if (this.epc != -1) {
            azg_12.vC(this.epc);
        }
        if (!this.stc.isEmpty()) {
            azg_12.aF((byte)this.stc.size());
            for (Map.Entry<fja, Integer> entry : this.stc.entrySet()) {
                azg_12.aG((short)entry.getKey().d());
                azg_12.vC(entry.getValue());
            }
        }
        return azg_12.bTe();
    }
    ...
    @Override
    @Generated
    public /* synthetic */ fiu_0 We(int n) {
        return this.Wd(n);
    }
}
